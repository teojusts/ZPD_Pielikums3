...
def scrape_price_and_address():
    try:
        ...
        # Updated regex to capture prices like 10, 00 EUR or 1000, 00 EUR
        match = re.search(r"(\d{1,3}(?:[\s,]?\d{3})*(?:[\.,]?\d{2})?)(\s*)(EUR|€|eiro|e?uro|\se?ur|\s?€)(/h)?", price_text, re.IGNORECASE)
...
def perform_initial_selections():
    try:
        ...
        # 3. Click the second dropdown to select and type "Jugla"
